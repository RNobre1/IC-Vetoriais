...
\textbf{Cenário B --- Busca com filtros de metadados:} mesma coleção do Cenário A, mas a consulta combina similaridade vetorial com um predicado sobre metadados, padrão típico de RAG corporativo (por exemplo, restringir a recuperação a uma categoria, área temática ou intervalo de datas). O fator que governa o custo e a qualidade da busca vetorial sob filtro é a \textit{seletividade} do predicado, isto é, a fração da coleção que o satisfaz, e não a natureza semântica do atributo; esse caráter \textit{predicate-agnostic} é evidenciado por \citeonline{patel2024acorn}. Por isso o filtro é operacionalizado de forma controlada: cada vetor recebe um atributo numérico sintético uniformemente distribuído e a consulta aplica o predicado de faixa correspondente, varrendo seletividades de 1\%, 10\%, 50\% e 100\% da base. O recall@K é medido contra uma busca exata restrita ao subconjunto que satisfaz o filtro, calculada de forma independente para cada nível de seletividade, o que evita penalizar o sistema por itens corretamente excluídos pelo predicado.

...
\textbf{Trade-off recall $\times$ throughput:} os três sistemas exibem o comportamento esperado de curva crescente de \textit{recall} e decrescente de QPS à medida que $ef\_search$ aumenta. O que difere entre eles é a inclinação: a amplitude de \textit{throughput} ao longo da curva é de 7,3 vezes no pgvector em 100 mil vetores, contra 2,3 no Weaviate e 1,7 no Qdrant. Sistemas de curva plana entregam desempenho previsível sob mudança de ponto de operação; o pgvector exige que a escolha de $ef\_search$ seja tratada como decisão de capacidade.
